...
def generate_enfo_req():
    ...
    param_steps = [['121/122', '6/to/360/by/6'], ['151/165/166/168/169/175/176/177/228', '0/to/360/by/24']]
    ...
    for date in dates:
        for param_step in param_steps:
            ...
            for i in range(1, 1+len(pf_numbers)):
                req_pfs.append([f'pf_{date[:10]}_{i}.grib', 'pf', 'number', date, area] + param_step + [pf_numbers[i-1]])

            all_req_kw.extend(req_pfs)
            all_req_kw.append(req_cf)

    req_df = pd.DataFrame(all_req_kw, columns=['file_id', 'type', 'number_kw', 'date', 'area', 'param', 'step', 'number'])
